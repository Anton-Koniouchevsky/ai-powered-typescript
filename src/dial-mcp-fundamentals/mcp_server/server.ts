...
server.addTool({
  name: 'searchUsers',
  description: 'Searches for users based on provided criteria.',
  parameters: z.object({
  ...
  execute: async ({ name, surname, email, gender }: { name?: string; surname?: string; email?: string; gender?: string }): Promise<string> => {
    const users = await userClient.searchUsers({ name, surname, email, gender });
...
server.addTool({
  ...
  execute: async (user: Omit<User, 'id'>): Promise<string> => {
    const newUser = await userClient.addUser(user);
...
server.addTool({
  ...
  execute: async ({ userId, updates }: { userId: string; updates: Partial<Omit<User, 'id'>> }): Promise<string> => {
    const updatedUser = await userClient.updateUser(userId, updates);
    return JSON.stringify(updatedUser);
  }
});

server.addPrompt({
  name: 'userSearchAssistantPrompt',
  description: 'Helps users formulate effective search queries',
  load: async () => `You are helping users search through a dynamic user database. The database contains 
...
server.addPrompt({
  name: 'userProfileCreationPrompt',
  description: 'Guides creation of realistic user profiles',
  load: async () => `You are helping create realistic user profiles for the system. Follow these guidelines 
